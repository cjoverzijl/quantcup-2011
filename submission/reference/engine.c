...
node * find_node(t_orderid id, list * l) {
  ...
  while (node_iter->data.id != id && node_iter != l->tail) {
...
/* IN: order_new: new order being added to the book
 ...
 OUT: */
void trade(t_order * order_new, node * node_old, list * l) { //printf("* trade %d %d\n", order_new->price, node_old->data.order.price);
  // shorthand
...
int hit_ask(t_price bid, t_price ask) {
  return bid >= ask;
}
int hit_bid(t_price ask, t_price bid) {
  return ask <= bid;
}

// cross as many shares as possible
int cross(t_order * order) { //printf("* cross\n");

  // which side
  int isask = is_ask(order->side);
  list * book = isask ? bids : asks;
  int (*cross_test)(t_price, t_price) = isask ? hit_bid : hit_ask;

  // trade through existing orders one-by-one
  node * book_iter = book->head->next;
  while (book_iter != book->tail && cross_test(order->price, book_iter->data.order.price)) {
...
int priority_ask(t_price ask_new, t_price ask_old) {
  return ask_new < ask_old;
}
int priority_bid(t_price bid_new, t_price bid_old) {
  return bid_new > bid_old;
}

void queue(t_order * order) { //printf("* queue %d\n", order->price);
  int isask = is_ask(order->side);
  list * book = isask ? asks : bids;
  int (*priority_test)(t_price, t_price) = isask ? priority_ask : priority_bid;

  node * book_iter = book->head->next;
  while (book_iter != book->tail && !priority_test(order->price, book_iter->data.order.price)) {
    book_iter = book_iter->next;
  }
  t_order_in o;
  o.order = *order;
...
t_orderid limit(t_order order) { //printf("\n\n* limit %d\n", nextid);
  if (!cross(&order))
    queue(&order);
  return nextid++;
}

void cancel(t_orderid orderid) { //printf("\n\n* cancel %d\n", orderid);
  // look for orderid in bids
  node * node = find_node(orderid, bids);
  if (node) {
    remove_node(bids, node);
  } else {
    // look for orderid in asks
    node = find_node(orderid, asks);
    if (node) {
      remove_node(asks, node);
    }
  }
}
